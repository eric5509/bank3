'use client'
import { transactionsLink, updateTransactionLink } from "@/lib/links";
...
export default function TransactionUpdateDropdown({ account, id, Status }: Props) {
    ...
    const index = status.findIndex((el) => el === storeData?.status);
    useEffect(() => {
        setActive(index)
    },[storeData2])
    ...
    const handleOptionClick = async (key: number) => {
        ...
        setOpen(false);
        try {
            const response = await axios.patch(`${updateTransactionLink}/${id}`, { status: status[key] });
