...
import { TAccount, TAccount2 } from "@/provider/slice/type";
...
export default function DropdownUpdateActive({ id, data }: Props) {
    ...
    const handleOptionClick = (key: number) => {
        setActive(key);
        setOpen(false);
        dispatch(editAccount({id, active: status[active]}))
    };
    useEffect(() => {
        setActive(data?.active ? 0 : 1)
